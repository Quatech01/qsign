...
async function migrate(standalone = false) {
  ...
  // Weekly hours target and pay rate per worker
  await pool.query(`
  ...
  // Pay period per company (weekly or monthly)
  await pool.query(`
    ALTER TABLE companies ADD COLUMN IF NOT EXISTS pay_period TEXT NOT NULL DEFAULT 'monthly';
  `);
  ...
  await pool.query(`
    ALTER TABLE users ADD COLUMN IF NOT EXISTS session_ver INTEGER NOT NULL DEFAULT 0;
  `);

  // Shift clock-in window per worker
  await pool.query(`
    ALTER TABLE users ADD COLUMN IF NOT EXISTS shift_start     TIME;
    ALTER TABLE users ADD COLUMN IF NOT EXISTS checkin_buffer  INTEGER DEFAULT 10;
  `);

  // Company timezone for shift time comparisons
  await pool.query(`
